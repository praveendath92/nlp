...
class Sentiment:
    ...
    def train_batch(self, iterations=iterations, iteration_offset=0, save_path="data/models/pretrained_lstm.ckpt"):
        self.init_tf_train()
        self.sess = tf.InteractiveSession()
        saver = tf.train.Saver(max_to_keep=5)
        self.sess.run(tf.global_variables_initializer())

        bar = progressbar.ProgressBar(max_value=iterations-iteration_offset)
        print("Training ...")
        for i in range(iteration_offset, iterations):
